...
import com.hollysys.hiadsp.uaa.oauth.client.ClientConstants;
...
import com.hollysys.hiadsp.uaa.util.system.JsonUtils;
...
import static com.hollysys.hiadsp.uaa.oauth.client.ClientConstants.REQUIRED_USER_GROUPS;

/**
 * A copy of JdbcClientDetailsService but with IdentityZone awareness
 */
public class MultitenantJdbcClientDetailsService extends ClientServicesExtension{
    ...
    public void updateClientDetails(ClientDetails clientDetails, String zoneId) throws NoSuchClientException {
        ...
        if (count != 1) {
            throw new NoSuchClientException("No client found with id = " + clientDetails.getClientId() + " in identity zone uaa");
        }
    }
	...
	parameters.put("identityZoneId", zoneId);
	listFactory.getList(DEFAULT_FIND_STATEMENT, parameters, rowMapper);
//	listFactory.getList(DEFAULT_FIND_STATEMENT, Collections.singletonMap("identityZoneId",zoneId), rowMapper);
        return listFactory.getList(DEFAULT_FIND_STATEMENT, parameters, rowMapper);
    }
